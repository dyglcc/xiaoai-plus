...
constexpr float kVadRmsThreshold = 300.0f;
constexpr int64_t kVadRmsThresholdSq =
    static_cast<int64_t>(kVadRmsThreshold * kVadRmsThreshold);
...
void LocalListener::AcceptPcm(const std::vector<uint8_t>& chunk) {
  ...
  // VAD: skip Zipformer inference on silence to reduce CPU and false positives.
  // Threshold ~1% full scale; well below far-field speech.
  {
    ...
    const int64_t vad_gate = kVadRmsThresholdSq * static_cast<int64_t>(samples);
    int64_t sum_sq = 0;
    for (size_t i = 0; i < samples; ++i) {
      sum_sq += static_cast<int64_t>(pcm[i]) * pcm[i];
      if (sum_sq >= vad_gate) {
        break;
      }
    }
